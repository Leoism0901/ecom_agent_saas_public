...
迭代版本：Day13 Step1~Step5，新增前端模拟流式输出生成器、分块延迟渲染逻辑，完整兼容原有LangGraph Agent适配层
...
4. 后端LangGraph Agent独立适配层（Step4）
   - _extract_log_entry：标准化LangChain消息对象转为前端可视化日志结构体，过滤无效过渡消息
   - call_agent_backend：前后端解耦异步适配入口，函数惰性导入规避循环依赖，内置沙盒兜底提示词，分层捕获异常
5. 前端流式输出适配层（Step5新增独立模块）
   - _stream_reply_chunks：生成器函数切割完整回复文本，分段休眠模拟LLM逐Token流式输出效果
   设计说明：前端模拟流式，无需改造后端run_agent，低成本实现打字机交互效果
6. 主体6:4双栏布局
   - 左栏60%：完整买家聊天交互面板（Step5升级流式渲染）
     1. 历史聊天气泡循环渲染
     2. 底部聊天输入框接收用户消息
     3. 全新交互流程：即时渲染用户消息→同步调用Agent→流式逐字渲染AI回复→落盘会话数据→页面刷新固化历史
...
4. 路径适配、状态管理、Agent适配、流式工具、UI渲染五层完全解耦，单一职责原则
5. 当前仅分层架构骨架与接口定义，无真实路径硬编码、完整执行逻辑、可对接后端的业务代码，无法直接部署运行
6. 页面文案、会话键名、系统提示词、流式分块参数、异常文案、布局比例全部抽离顶层常量统一管控
...
# ============================================================================
# 全局静态常量统一托管层
# 租户数据源、页面配置、侧边栏文案、聊天面板文案、Agent兜底提示词、异常文案、流式渲染参数、布局比例、会话State键名全部抽离
# ============================================================================
...
# Step5 流式输出专属常量：单块字符长度、块间隔延迟
_STREAM_CHUNK_SIZE: int = 0
_STREAM_CHUNK_DELAY: float = 0.0
...
# --------------------------------------------------------------------------
# Step5 前端流式输出模拟适配层（独立工具模块，不改动后端逻辑）
# --------------------------------------------------------------------------
def _stream_reply_chunks(text: str) -> Generator[str, None, None]:
    """
    生成器工具函数，将完整AI回复文本分段切割、延时产出，配合st.write_stream实现打字机流式效果
    实现方案：前端模拟流式输出，无需改造后端LangGraph流式接口，低成本优化交互体验
    底层逻辑：按固定字符长度分割文本块，每块产出后休眠固定时长模拟LLM Token传输延迟
    Args:
        text: Agent返回完整AI回复文本
    Yields:
        分段文本块，供前端增量渲染
    """
    pass
...
def build_left_panel(col_handle: st.delta_generator.DeltaGenerator) -> None:
    """
    左侧买家完整聊天交互面板渲染函数（Step5升级前端流式打字机输出）
    分层渲染逻辑：面板标题、历史聊天气泡循环渲染、底部聊天输入框
    标准化交互闭环流程：用户提交输入→快照历史上下文→即时渲染用户气泡→加载占位→异步调用Agent适配层
    →捕获分层异常生成兜底回复&错误日志→流式逐块渲染AI回复→落盘用户/AI消息与全链路日志→页面强制刷新固化历史
    核心优化点：先渲染再持久化会话，避免历史列表提前加载完整文本破坏流式视觉效果
